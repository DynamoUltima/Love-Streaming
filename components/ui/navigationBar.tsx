...
const NavigationBar = () => {
  const Menus = [
    { name: "Home", path: '/', icon: <div><HomeIcon className="w-6 h-6  " /></div>, dis: "translate-x-0" },
    { name: "Search", path: '/search',icon: <MagnifyingGlassIcon className="w-6 h-6 " />, dis: "translate-x-16" },
    { name: "Browse", path: '/browse', icon: <Bars3Icon className="w-6 h-6  " />, dis: "translate-x-32" },
    { name: "History", path: '/history', icon: <ClockIcon className="w-6 h-6 " />, dis: "translate-x-48" },
    {
      name: "Profile",path: '/profile', icon: <div className="relative w-6 h-6  bg-white rounded-full overflow-clip ">
